...
namespace SignalRChatRoom.Hubs
{
    ...
    public class GroupsHub : Hub
    {
        ...
        public void SendMessage(string roomName, string message)
        {
            ...
            Clients.Group(roomName, new string[0]).sendMessage(roomName, message + " " + DateTime.Now);
        }

        // 发送图片
        public void SendImage(string roomName, IEnumerable<ImageData> images)
        {
            foreach (var item in images ?? Enumerable.Empty<ImageData>())
            {
                if (String.IsNullOrEmpty(item.Image)) continue;
                Clients.Group(roomName, new string[0]).receiveImage(roomName, item.Image + " " + DateTime.Now ); // 调用客户端receiveImage方法将图片进行显示
            }
        }
        #endregion 
    }
