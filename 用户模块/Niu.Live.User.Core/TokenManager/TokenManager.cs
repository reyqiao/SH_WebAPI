...
namespace Niu.Live.User.Core
{
    ...
    public class TokenManager
    {
        ...
        public static bool ValidateMobileUserToken(string userToken, out TokenUserInfo user)
        {
            user = null;
            try
            {
                ...
                if (!string.IsNullOrEmpty(userToken))
                {
                    ...
                    if (userInfoArray.Length == 9)
                    {
                        ...
                        {
                            userId = ObjectConvert.ChangeType<long>(userInfoArray[1], 0L),
                            nickName = userInfoArray[3],
                            tokenType = (tokenType)ObjectConvert.ChangeType<int>(userInfoArray[0], 0),
        ...
        /// <returns>true/false</returns>
        public static bool GenerateMobileUserToken(TokenUserInfo user, out string userToken)
        {
            ...
            if (user != null && user.userId > 0 && !string.IsNullOrEmpty(user.nickName))
            {
                ...
                sb.Append(user.channelId.ToString());
                sb.Append(Separator);
        ...
        /// <returns></returns>
        public static string GenerateMobileUserToken(TokenUserInfo user)
        {
            string userToken = string.Empty;
            GenerateMobileUserToken(user, out userToken);
            return userToken;
        }
